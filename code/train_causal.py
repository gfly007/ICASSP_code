...
def train_causal_syn(train_set, val_set, test_set, model_func=None, args=None):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, loss_c, loss_o, loss_co, train_acc_o = train_causal_epoch(model, optimizer, train_loader, device,
                                                                              args)
        ...
        print(
            "BIAS:[{:.2f}] | Model:[{}] Epoch:[{}/{}] Loss:[{:.4f}={:.4f}+{:.4f}+{:.4f}] Train:[{:.2f}] val:[{:.2f}] Test:[{:.2f}] | Update Test:[co:{:.2f},c:{:.2f},o:{:.2f}] at Epoch:[{}] | lr:{:.6f}"
                .format(args.bias,
                        args.model,
                        epoch,
                        args.epochs,
...
def train_causal_real(dataset=None, model_func=None, args=None):
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(zip(*k_fold(dataset, args.folds, args.epoch_select))):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            train_loss, loss_c, loss_o, loss_co, train_acc = train_causal_epoch(model, optimizer, train_loader, device,
                                                                                args)
            ...
            print(
                "Causal | dataset:[{}] fold:[{}] | Epoch:[{}/{}] Loss:[{:.4f}={:.4f}+{:.4f}+{:.4f}] Train:[{:.4f}] Test:[{:.2f}] Test_o:[{:.2f}] Test_c:[{:.2f}] (RG:{:.2f}) | Best Test:[{:.2f}] at Epoch:[{}] | Test_o:[{:.2f}] Test_c:[{:.2f}]"
                    .format(args.dataset,
                            fold,
                            epoch, args.epochs,
        ...
        print(
            "syd: Causal fold:[{}] | Dataset:[{}] Model:[{}] | Best Test:[{:.2f}] at epoch [{}] | Test_o:[{:.2f}] Test_c:[{:.2f}] (RG:{:.2f})"
                .format(fold,
                        args.dataset,
                        args.model,
                        best_test_acc * 100,
    ...
    train_acc, test_acc, test_acc_c, test_acc_o = tensor(train_accs), tensor(test_accs), tensor(test_accs_c), tensor(
        test_accs_o)
    ...
    print("=" * 150)
    print(
        'sydall Final: Causal | Dataset:[{}] Model:[{}] seed:[{}]| Test Acc: {:.2f}±{:.2f} | OTest: {:.2f}±{:.2f}, CTest: {:.2f}±{:.2f} (RG:{:.2f}) | [Settings] co:{},c:{},o:{},harf:{},dim:{},fc:{}'
            .format(args.dataset,
                    args.model,
                    args.seed,
                    test_acc_mean * 100,
                    ...
                    args.fc_num))
    print("=" * 150)
    with open(model_name + "_" + args.dataset + ".txt", 'a') as f:
        f.write(str(test_acc_mean * 100) + "  " + str(test_acc_std * 100))
...
def train_causal_epoch(model, optimizer, loader, device, args):
    ...
    for it, data in enumerate(loader):
        ...
        c_logs, o_logs, co_logs, c_pres, edge_att, node_att = model(data, eval_random=args.with_random)
        ...
        loss = args.c * c_loss + args.o * o_loss + args.co * co_loss + 0.001 * node_grad_loss + 0.001 * edge_grad_loss  # +0.2*cs_loss
