...
class CausalGCN(torch.nn.Module):
    ...
    def forward(self, data, eval_random=True):
        ...
        #     self.xos.append(xo)
        #
        # xc_ = self.xcs[-1] * 1 + 0.6*self.xcs[-2] + 0.3*self.xcs[-3]
        # xo_ = self.xos[-1] * 1

        xc_logis = self.context_readout_layer(xc)  # * i / len(self.xos)
        xo_logis = self.objects_readout_layer(xo)  # * i / len(self.xos)
        xco_logis = self.random_readout_layer(xc, xo, eval_random=eval_random)  # * i / len(self.xos)

        xc_pres = self.objects_readout_layer(xc)

        return xc_logis, xo_logis, xco_logis, xc_pres, edge_att, node_att
...
class CausalGIN(torch.nn.Module):
    ...
    def __init__(self, num_features,
        ...
        for i in range(num_conv_layers):
            ...
            self.convs.append(tgnn.GINConv(MLP([hidden, hidden, hidden]))
                # GINConv(Sequential(
    ...
    def forward(self, data, eval_random=True, train_type="base"):
        ...
        xc_pres = self.objects_readout_layer(xc, train_type)
